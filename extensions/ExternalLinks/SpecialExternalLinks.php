...
class ExternalLinks extends SpecialPage {
	...
	*/
	...
	function ELdoExternalLinks() {
		...
		$top .= '<fieldset><legend>' . wfMessage( 'el-filterButton' )->text() . '</legend>
						 <form name="ELform" id="ELform" style="display:inline" enctype="multipart/form-data" method="GET" action="'.$wgScriptPath.'/index.php/'.$wgTitle.'">
							 ...
							 <td align="right"><label for="filterURL">'.wfMessage('el-filterURL')->text() . '</label></td>
							 <td><input type="text" name="filterURL" id="filterURL" size="40" value="'.$filterURL.'">&nbsp;							 
							 <label for="filterURLnot">'.wfMessage('el-filterNot')->text() . '</label><!--
							 ...
							 --><label for="showEmail">'.wfMessage('el-showEmail')->text() .'</label>
							 </td>
		...
		if($wgELvalidationMode != NULL) {
			$top .= ' 
							 ...
							 --><label for="checkResponse">'.wfMessage('el-checkResponse')->text() .'</label></td>
							';
		}
		if($wgELenableSessionStoring != NULL) {
			$top .= '
							 ...
							 --><label for="storeSession">'.wfMessage('el-storeSession')->text().'</label></td>
							 </tr><tr>
							 <td>'.wfMessage('el-lastValidationOfURL')->text().'</td>
							 <td><input type="text" name="lastValidation" id="lastValidation" size="3" value="'.$lastValidation.'">
							 '.wfMessage('el-lastValidationDays')->text().'&nbsp; ('.wfMessage('el-lastValidationInfo')->text().')
							 </td>
							';
		}
		if( is_numeric($pageID) ) { //if pageID in URL parameter (so it can be removed for new query), but independent from $wgELtoolboxLink
			$top .= '</tr><tr>
							 <td align="right">'.wfMessage('el-pageID')->text().':</td>
							 <td><input type="text" name="pageID" id="pageID" size="7" value="'.$pageID.'"></td>
							 ';
		}
		$top .= '</tr><tr><td></td><td><input type="submit" class="ELsubmitButton" style="margin-top:8px" value="'.wfMessage('el-submit')->text().'"></td>
						 </tr></table>
						 </form></fieldset>';
		$wgOut->addHTML( $top );
		...
		$navi = '';
		$naviReturn = '';
		$text = wfMessage('search-nonefound', '')->text(); //MW default msg
		...
		if( $numThis > 0 ) {
			...
			$naviInfo = '('.($offset + 1).'&minus;'.($offset + $numThis).' '.wfMessage('el-nav-outOf')->text().' <strong>'.$numAll.'</strong>)';
			// Prev/First
			if( $offset > 0 ) {
				$navi .= '(<a href="'.$_SERVER['PHP_SELF'].'?offset='.($offsetNavi).''.$params.'">'.wfMessage('el-nav-prev')->text().' '.$limit.'</a>';
				$first = '<a href="'.$_SERVER['PHP_SELF'].'?offset=0'.$params.'">'.wfMessage('el-nav-first')->text().'</a>';
			} else {
				$navi .= '('.wfMessage('el-nav-prev')->text().' '.$limit;
				$first = wfMessage('el-nav-first')->text();
			}
			$navi .= ') (';
			// Next/Last
			if( ($offset + $numThis) < $numAll ) {
				$navi .= '<a href="'.$_SERVER['PHP_SELF'].'?offset='.($offset + $limit).''.$params.'">'.wfMessage('el-nav-next')->text().' '.$limit.'</a>';
				$last = '<a href="'.$_SERVER['PHP_SELF'].'?offset='.($numAll - $limit).''.$params.'">'.wfMessage('el-nav-last')->text().'</a>';
			} else {
				$navi .= wfMessage('el-nav-next')->text().' '.$limit;
				$last = wfMessage('el-nav-last')->text();
			}
			$navi .= ')&nbsp; ';
			$navi .= $naviInfo;
			$navi .= '&nbsp; ('.$first.' | '.$last.')';
			...
			$text .= '<tr><th>';
			$text .= wfMessage('el-th-page')->text();
			$text .= '</th><th>';
			$text .= wfMessage('el-th-url')->text();
			$text .= '</th>';
			if( $checkResponse != NULL && $wgELvalidationMode != NULL ) {
				$text .= '<th>'.wfMessage('el-serverResponse')->text().'</th>';
			}
			$text .= '</tr>';
			foreach ( $res as $row ) {
				...
				$text .= '<tr>';
				$text .= '<td class="ELtablePage plainlinks '.$wgELpageLinkClass.'" title="'.wfMessage('edit')->text().'">[{{fullurl:'.$title.'|action=edit&preview=yes&minor=1&summary='.urlencode(wfMessage('el-edit-summary')->text()).'#toolbar}} '.$title.']</td>';
