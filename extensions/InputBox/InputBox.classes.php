...
class InputBox {
	...
	public function getSearchForm2() {
		...
		$htmlOut .= Xml::element( 'input',
			array(
				...
				'class' => 'mw-searchInput',	/* WR: added generic searchbox class mw-searchInput, for search suggestions */
				'dir' => $this->mDir,
			)
		);
		$htmlOut .= Xml::element( 'input',
	...
	/**
	 * Generate search form version 3 - bootstrap-themed, by Dror, for Kol-Zchut
	 */
	public function getSearchForm3() {
		// Use button label fallbacks
		if ( !$this->mButtonLabel ) {
			$this->mButtonLabel = wfMessage( 'tryexact' )->text();
		}
		...
		if ( isset( $this->mLabelText ) && strlen( trim( $this->mLabelText ) ) ) {
			$this->mLabelText = $this->mParser->recursiveTagParse( $this->mLabelText );
			$htmlLabel = Html::openElement( 'label', array( 'for' => 'bodySearchInput' . $id ) );
			$htmlLabel .= $this->mLabelText;
		...
		$htmlOut .= Html::openElement( 'div',
			array(
				'class' => 'bodySearchWrap input-group',
			)
		);
		$htmlOut .= $htmlLabel;
		$htmlOut .= Html::element( 'input',
			array(
				...
				'id' => 'bodySearchInput' . $id,
				'dir' => $this->mDir,
				'class' => 'form-control input-lg mw-searchInput ',
				'placeholder' => $this->mPlaceholderText,
			)
		);
		...
		$htmlOut .= Html::openElement( 'button',
			array(
				...
				'class' => 'btn btn-default btn-lg bodySearchBtnGo' . $id,
			)
		);
        $htmlOut .= Html::openElement( 'span',
            array(
